...
public class Main {
    static File file;
    final static String fileName = System.getenv().get("Bands");
    ...
    public static File getFile () {
        ...
        if (fileName == null) {
            ...
            filePath.mkdir();
            file = new File(filePath + "\\save.csv");
            try {
                boolean exists = file.createNewFile();
                System.out.println(exists ? "Файл создан!" : "Файл уже существует!");
                if(!exists)
                    Methods.upload(file);
            } catch (IOException e) {
                e.printStackTrace();
    ...
    public static boolean checkMethod(String method) throws NullPointerException {
        boolean flag = true;
        switch (method){
            case "help":{
                Methods.help();
